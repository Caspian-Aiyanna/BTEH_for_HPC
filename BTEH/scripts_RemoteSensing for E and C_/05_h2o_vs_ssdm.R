...
.this_file <- function() {
  ...
  if (length(filearg)) return(normalizePath(sub("^--file=", "", filearg), mustWork = FALSE))
...
save_bar(plot_df, "jaccard_hotspot_q",
         ...
         "jaccard_bar.png", bar_dir, clamp01 = TRUE, digits = 3)
...
df <- read_csv("BTEH/results/compare/h2o_vs_ssdm/between_method_metrics.csv")

names(df) <- c("Dataset", "Pearson_r", "Spearman", "RMSE", "MAE", "Jaccard_0.75")

plot_metric <- function(df, metric, clamp01 = FALSE) {
  ggplot(df, aes(x = reorder(Dataset, !!sym(metric)), y = !!sym(metric))) +
    geom_col(fill = "#1f77b4", width = 0.8) +
    geom_text(aes(label = round(!!sym(metric), 3)), hjust = -0.1, size = 4, color = "black") +
    coord_flip(clip = "off") +
    scale_y_continuous(limits = if (clamp01) c(0,1) else NULL) +
    labs(
      title = paste0("Between-method ", metric, " (SSDM vs H2O)"),
      x = NULL, y = NULL
    ) +
    theme_minimal(base_size = 13)
}

outdir <- "BTEH/results/compare/h2o_vs_ssdm/"
ggsave(file.path(outdir, "pearson_bar.png"),       plot_metric(df, "Pearson_r", TRUE), width = 7, height = 4)
ggsave(file.path(outdir, "spearman_bar.png"),      plot_metric(df, "Spearman", TRUE), width = 7, height = 4)
ggsave(file.path(outdir, "jaccard_bar.png"),       plot_metric(df, "Jaccard_0.75", TRUE), width = 7, height = 4)
ggsave(file.path(outdir, "rmse_bar.png"),          plot_metric(df, "RMSE", FALSE), width = 7, height = 4)
ggsave(file.path(outdir, "mae_bar.png"),           plot_metric(df, "MAE", FALSE), width = 7, height = 4)

cat("Saved all 5 metric plots to", outdir, "\n")
