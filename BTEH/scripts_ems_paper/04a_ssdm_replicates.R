...
dir_ensure(paths$results_ssdm)
dir_ensure(paths$plans)
dir_ensure(paths$logs)
...
for (f in sort(all_rep)) {
  ...
  if (length(x_vars) == 0L) {
    log_line(sprintf("Skip %s (rep %d) — no usable predictors at presences", sp, rep_idx), logf)
    next
  }
  ...
  esdm <- tryCatch(
    ...
    error = function(e) {
      log_line(sprintf(
        "ensemble_modelling() error for %s (rep %d): %s",
        sp, rep_idx, conditionMessage(e)
      ), logf)
      NULL
    }
  )
  ...
  if (!is.null(esdm@uncertainty) && inherits(esdm@uncertainty, "RasterLayer")) {
    ...
    raster::writeRaster(esdm@uncertainty,
      filename = unc_tif, format = "GTiff",
      overwrite = TRUE, NAflag = -9999, datatype = "FLT4S"
    )
  }
    ...
    if (!is.null(vi_df) && nrow(vi_df) > 0) {
      ...
      if ("Importance" %in% names(vi_df)) {
        p <- ggplot(vi_df, aes(x = reorder(Variable, Importance), y = Importance)) +
          ...
          labs(
            title = paste("Variable importance —", sp, "(rep", rep_idx, ")"),
