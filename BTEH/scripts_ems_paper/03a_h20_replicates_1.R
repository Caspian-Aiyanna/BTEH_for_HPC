...
# Inputs : data/occ/replicates_DBSCAN/<SP>_rep<k>.csv
...
# --- robust sourcing regardless of CWD (identical pattern) ---
.this_file <- function() {
...
# --- CLI (mirrors 03_h2o_train.R; only new flag is --input) ---
opt <- list(
  make_option(c("--run"),              type = "character", default = NULL,   help = "Run tag: A or B [default: %default]"),
  make_option(c("--mode"),             type = "character", default = NULL,  help = "REPRO or FAST (overrides config.yml)"),
  make_option(c("--species"),          type = "character", default = NULL,  help = "Optional single species (e.g., E3A) to filter replicates"),
  make_option(c("--input"),            type = "character", default = NULL,  help = "Replicate CSV folder [default: data/occ/replicates_DBSCAN]"),
...
# --- normalize & validate --run (exact behavior) ---
opts$run <- toupper({
  x <- opts$run
...
# --- config / mode (exact pattern) ---
cfg <- read_config()
if (!is.null(opts$mode)) cfg$mode <- toupper(opts$mode)
mode <- set_mode(cfg)

# --- paths (follow the same config keys as 03_h2o_train.R) ---
paths <- list(
...
# Replicates directory default
rep_dir <- opts$input %||% file.path("data","occ","replicates_DBSCAN")

dir_ensure(paths$results_h2o); dir_ensure(paths$plans); dir_ensure(paths$logs)
logf <- file.path(paths$logs, sprintf("03a_h2o_replicates_%s.log", opts$run))
log_line(sprintf("Starting 03a_h2o_replicates.R  (mode=%s, run=%s)", mode, opts$run), logf)

# --- choose env dir by run tag (identical) ---
env_dir <- if (opts$run == "A") paths$envi_after else paths$envi_before
...
log_line(sprintf("Env dir: %s (%d layers)", env_dir, nlyr(Renv)), logf)
...
log_line(sprintf("Replicates dir: %s (%d files)", rep_dir, length(all_rep)), logf)
...
if (!is.null(opts$species)) {
  ...
  if (!length(all_rep)) {
    ...
    h2o.shutdown(prompt = FALSE); quit(status = 0)
  }
}
...
for (f in sort(all_rep)) {
  ...
  # Resume guard (mirrors 03, plus predicted tif)
  if (skip_if_done(out_dir, c("leader_model.txt", "metrics_in_sample.csv",
                              paste0("prediction_", sp, "_rep", rep_idx, ".tif")))) {
  ...
  # Deterministic background (species + rep salt)
  ...
  # AutoML with safe fallback (seed salted by rep, but deterministic)
  aml <- tryCatch(
    h2o.automl(
      x = x_vars, y = "pa",
      training_frame   = hf,
  ...
  if (!is.null(aml)) {
    ...
    if (!is.null(lb) && nrow(lb) > 0) {
      ...
      log_line(sprintf("Empty AutoML leaderboard for %s (rep %d) — switching to fallback GBM.", sp, rep_idx), logf)
    }
  } else {
    log_line(sprintf("AutoML returned NULL for %s (rep %d) — switching to fallback GBM.", sp, rep_idx), logf)
  }
  ...
  # Save leader model + info (identical metadata + replicate tag)
  ...
  # Raster prediction (chunked; same helper)
  pred_r <- predict_raster_h2o(Renv[[x_vars]], leader, block_rows = 50000)
  terra::writeRaster(pred_r,
                     filename = file.path(out_dir, sprintf("prediction_%s_rep%d.tif", sp, rep_idx)),
                     overwrite = TRUE)
