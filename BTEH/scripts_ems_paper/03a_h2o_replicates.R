#!/usr/bin/env Rscript
# H2O AutoML Training on DBSCAN Replicates
# Trains H2O models on replicate occurrence datasets for uncertainty analysis
# Input:  replicates_DBSCAN/<SPECIES>_rep<N>.csv
# Output: results/H2O/<RUN>/<SPECIES>/replicates/rep<N>/
...
opt <- list(
  ...
  make_option(c("--input"), type = "character", default = NULL, help = "Folder of replicate CSVs (default: <project_root>/BTEH/data/occ/replicates_DBSCAN)"),
...
for (f in sort(all_rep)) {
  ...
  cls_tab <- table(df$pa)
  if (nrow(df) < n_min) {
    log_line(sprintf("Skip %s (rep %d) — too few rows after drop_na(): %d < %d", sp, rep_idx, nrow(df), n_min), logf)
    next
  }
  if (length(cls_tab) < 2 || any(cls_tab < 10)) {
    log_line(sprintf(
      "Skip %s (rep %d) — class imbalance after drop_na() (counts: %s)",
      sp, rep_idx, paste(names(cls_tab), as.integer(cls_tab), sep = "=", collapse = ", ")
    ), logf)
    next
  }
  if (length(x_vars) == 0L) {
    log_line(sprintf("Skip %s (rep %d) — no usable predictors after pruning/NZV", sp, rep_idx), logf)
    next
  }
